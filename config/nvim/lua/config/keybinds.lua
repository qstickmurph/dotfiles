...
return {
  ...
  { -- Project/Files Group
    ...
    },
    {
      "<Leader>pd",
      function()
        local bufferParentDir = vim.fn.expand("%:p:h")
        if bufferParentDir == "" then
          bufferParentDir = vim.fn.getcwd()
        end
        Snacks.picker.explorer({
          auto_close = true,
          layout = {
  ...
  { -- Debugger Group
    ...
    },
    {
      ...
      desc = "Eval",
  -- Watch a test (re-runs on file save)     mode = { "n", "x" },
    },
  },
  { -- Testing Group
    "<Leader>t",
    ...
    },
    {
      "<Leader>tP",
      function()
        require("neotest").run.run(vim.fn.input("Project path: ", vim.fn.getcwd() .. "/", "dir"))
      end,
